...
void main() {
  ...
  float centerU = (UVMax.x + UVMin.x) * 0.5;
  float centerV = (UVMax.y + UVMin.y) * 0.5;
  float halfU = max((UVMax.x - UVMin.x) * 0.5, 1e-5);
  float halfV = max((UVMax.y - UVMin.y) * 0.5, 1e-5);
  float rangeAspect = halfU / halfV;
  if (uAspect > rangeAspect) halfU = halfV * uAspect;
  else halfV = halfU / uAspect;
  float uOffset = centerU + (uv.x - 0.5) * 2.0 * halfU;
  float vOffset = centerV + (uv.y - 0.5) * 2.0 * halfV;
